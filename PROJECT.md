...
| `qs-ytmd-titlebar` | **Runs as root.** Rebuilds `/opt/ytmdesktop/resources/app.asar` with the app's own title-bar colors patched (see §8). Keeps `app.asar.qs-backup` and always patches from it, so re-runs don't stack; `--restore` puts the original back. Must be re-run after every app update; `qs-theme-ytmd` prints a reminder once a backup exists. |
...
| `~/.config/quickshell/palettes.json` | **Every theme's colors, in one place.** Per theme: the 21 UI colors, `label`, plus `gtkThemeName` / `gtkAccentName` / `kvantumTheme` / `nvimScheme`. Watched live by `Theme.qml` **and by nvim** (§7); read by every script via `qs-palette`. NB the GTK theme key is `gtkThemeName`, not `gtkTheme` — `qs-palette env` would otherwise export `GTK_THEME` and re-break gotcha #25. `nvimScheme` is `lemon` for every theme now: the editor generates itself from these same colors instead of pulling in a per-theme plugin, so the key only still exists as the escape hatch for a theme that wants a real upstream colorscheme. |
| `~/.config/quickshell/theme.txt` | Active theme name. Watched live by `Theme.qml`. |
| `~/.config/quickshell/settings.json` | **The control centre's effect toggles**, a flat object of booleans (`animations`, `blur`, `shadows`, `transparency`, `gaps`). Written only by `qs-settings`, watched live by `SettingsService.qml`, and pattern-matched (no JSON library in the compositor config) by `config/hypr/config/toggles.lua`. A missing key means on. **Not in the repo** — a fresh install has no file and therefore the normal look. |
| `~/.config/quickshell/bar.json` | **Which parts of the bar are drawn.** A flat object of booleans (`date`, `clock`, `focusedApp`, `workspaces`, `cava`, `media`, `stats`, `volume`, `tray`, `notifications`) written and read only by `BarSettings.qml`. A missing key means on, except `cava`. Switching something off also **stops its polling** — the five stat processes every two seconds are the most expensive thing the bar does. **Not in the repo.** |
...
| `~/.local/share/quickshell/colors.json` | **The picked-colour history**, a plain array of hex strings, newest first, capped at 24. Written only by `qs-color`, watched live by `ColorService.qml`. Also the channel that carries a fresh pick into the bar, since `ipc call` cannot take arguments (§1 #1). |
...
colorscheme per theme, but by reading `palettes.json` directly.** There is a
single colorscheme, `lemon`, and it renders whichever theme `theme.txt` names.
Adding a theme is still one block in `palettes.json` and nothing else — the
...
| `colors/lemon.lua` | The colorscheme itself. Drops both modules from `package.loaded` on every load, so a theme switch or a hand edit of `palettes.json` is picked up rather than served from cache. Publishes `g:lemon_palette`. |
...
| `lua/plugins/colorscheme.lua` | Loads it and **watches `theme.txt` and `palettes.json` with `vim.uv.new_fs_event`**, so `Super+Ctrl+Z` restyles every open editor live, like the bar and kitty. The watcher re-arms after each event: a theme switch can replace the file rather than truncate it, and the old handle would then be watching an inode nothing writes to. |
