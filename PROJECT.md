...
| 67 | **`ObjectModel.values.length` is a number, not a notification.** A binding on it does not re-run when the model gains or loses an object, so the overview's cards were laid out from the window count as it was when the column was built — dragging a window onto a workspace left the ones already there at their old size until something *else* forced a rebuild, which visiting that workspace did (it flips `active`, which rebuilds the columns). Take counts from the `Repeater` instead: `onCountChanged: column.windowCount = repeater.count`. The Repeater does track its model; the length of a snapshot list does not. |
| 66 | **An `IconImage` whose `anchors` are swapped at runtime comes back at the icon file's own size.** The overview's card icon switched between `anchors.left` and `anchors.horizontalCenter` depending on whether the card was wide enough for a title, and cards that had just been resized rendered a **72px** icon lying across the card — measured `width: 72` while `implicitWidth` was 0 and the binding said `Math.min(16, parent.height - 4)` with a 20px-high parent, so the width binding was simply no longer in effect. Fixed by moving the icon with `x` instead of swapping anchors, giving it an explicit `implicitSize`, and not reading `parent.height` for its size — the exact mechanism was never isolated, but all three together hold. Reproduce with two windows on a workspace and move one away with the overview open. |
...
**A column's number is a handle too.** Dragging it takes the whole workspace: every window on it is dispatched to whatever the drop lands on, one `window.move` each (Hyprland has no "empty this workspace into that one"). Dropped on a number that does not exist yet — the spare column or a gap — the workspace is emptied, folded up by Hyprland, and the same windows are sitting on the new number, which is a rename in everything but name. A plain click on the number focuses that workspace and closes the overview.

**Previews are one frame per opening** (`live: false` + `captureFrame()`), not a live feed — eight windows would otherwise be eight capture streams at monitor rate for as long as the panel is up. They are re-taken 160ms after a column's window count changes, because Hyprland re-tiles a workspace when a window joins or leaves it and the frame from before is the wrong shape.
...
- Card sizes come off a window count the Repeater pushes in, not `toplevels.values.length` (gotcha #67), and the card icon is placed with `x` rather than by swapping anchors (gotcha #66).
...
| `WindowSwitcherWindow.qml` | The overlay: a row of cards — **a preview of the window**, its icon and app name under it, workspace badge in the corner — plus the selected window's title. Same capture path as the overview (`ScreencopyView`, one frame per opening); the icon stands in until the frame lands, so nothing waits on it and Alt+Tab still comes up immediately. The list stays the service's, in MRU order, and each card finds its wayland handle by matching its address against `Hyprland.toplevels` — the service's entries do not carry one. Commit calls `hl.dsp.focus({ window = "address:0x…" })`, which switches to that window's workspace as well. Focus is dispatched **before** the panel drops its keyboard grab so the old window can't get focus back in between. Animations are deliberately short (90–130ms) — Alt+Tab has to feel instant. |
