...
| — | **`input:mouse_refocus = false`** (`config/hypr/config/input.lua`) is also set: with `follow_mouse = 1`, hover-refocus otherwise fires on *any* pointer movement, not just crossing into a different window, which could re-steal focus from a keybind-driven change on the next jitter. Its official description only exists in the Hyprland binary, not the Lua stub's doc comments: `strings /usr/bin/Hyprland \| grep mouse_refocus`. |
...
| `LauncherSystemProvider.qml` | Lock / log out / suspend / reboot / shut down / reload shell / audio / btop. |
